...
CREATE TABLE context_audit_log (
    ...
    context_type VARCHAR(20) NOT NULL,        -- 'SESSION' or 'WORKFLOW'
    context_key VARCHAR(100),                 -- NULL for session, workflow name for workflow (e.g., 'Examination')

    -- Event information
    event_type VARCHAR(20) NOT NULL,          -- 'CREATED', 'UPDATED', 'CLEARED'
    ...
    -- Constraints
    CONSTRAINT chk_context_type CHECK (context_type IN ('SESSION', 'WORKFLOW')),
    CONSTRAINT chk_event_type CHECK (event_type IN ('CREATED', 'UPDATED', 'CLEARED'))
...
CREATE INDEX idx_context_audit_timestamp ON context_audit_log(event_timestamp DESC);

-- Composite index for common filtered time queries
CREATE INDEX idx_context_audit_user_timestamp ON context_audit_log(user_id, event_timestamp DESC);

-- Composite index for context type + timestamp queries
CREATE INDEX idx_context_audit_type_timestamp ON context_audit_log(context_type, event_timestamp DESC);
